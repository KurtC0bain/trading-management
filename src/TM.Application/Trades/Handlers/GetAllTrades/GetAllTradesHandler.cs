...
{
    public class GetAllTradesHandler(IRepositoryBase<Trade> repository, IMapper mapper) : IRequestHandler<GetAllTradesQuery, Result<InternalError, List<TradeResponse>>>
    {
        ...
        public async Task<Result<InternalError, List<TradeResponse>>> Handle(GetAllTradesQuery request, CancellationToken cancellationToken)
        {
            ...
            return _mapper.Map<List<TradeResponse>>(trades);
        }
    }
}
